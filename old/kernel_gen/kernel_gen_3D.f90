...
!kernel(i,j): response at i of source at j
  double precision, allocatable :: kernel(:,:)   ! 2d bruteforce kernel (nx*nw)^2 count
  ...
  allocate (mesh%dw(mesh%nw), mesh%DIP_W(mesh%nw),   &
    ...
    kernel(mesh%nw*mesh%nx,mesh%nw*mesh%nx))
  ...
  do i = 1,mesh%nw*mesh%nx
    do j = 1,mesh%nw*mesh%nx
      call compute_kernel(LAM,MU,mesh%x(j),mesh%y(j),mesh%z(j),  &
                      ...
                      mesh%x(i),mesh%y(i),mesh%z(i),mesh%dip(i),IRET,tau)
      if (IRET == 0) then
        kernel(i,j) = tau
      else
