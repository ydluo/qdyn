...
switch mode
  ...
  case 'run',
    ...
    % export qdyn.in
    fid=fopen('qdyn.in','w');
    fprintf(fid,'%.15g %.15g %u %.15g    mu*, V*, state law, sigma(Pa)\n',MU_SS,V_SS,THETA_LAW,SIGMA);
    fprintf(fid,'%.15g %.15g %.15g    mu, cs, W\n',MU,VS,W);
    fprintf(fid,'%.15g %u %.15g    L, nstop, tmax \n',L,NSTOP,TMAX);
    fprintf(fid,'%.15g %.15g    Tper, Aper\n',TPER,APER);
    fprintf(fid,'%u %u    nxout, ntout \n',NXOUT,NTOUT);
    fprintf(fid,'%.15g %.15g %.15g    dt_try, dtmax, accuracy\n',DTTRY,DTMAX,ACC);
...
ACC   = rdat(3);
fclose(fid);
[X,A,B,DC,V_0,TH_0]=textread(name,'','headerlines',6);
