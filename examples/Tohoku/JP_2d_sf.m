...
d7=50e3;
d8=55e3;
db=65e3;     %bottom of simulation zone (depth in m)
aa0=0.01;    %p.A at < 55km
aa1=0.02;    %p.A at db
...
p.B(1:ceil((db-d7)/dz0))=p.A(1:ceil((db-d7)/dz0)).*linspace(0.1,ba0,numel(1:ceil((db-d7)/dz0)));
p.B(ceil((db-d7)/dz0)+1:ceil((db-d6)/dz0))=p.A(ceil((db-d7)/dz0)+1:ceil((db-d6)/dz0)).*ba0;
p.B(ceil((db-d6)/dz0)+1:ceil((db-d5)/dz0))=p.A(ceil((db-d6)/dz0)+1:ceil((db-d5)/dz0)).*linspace(ba0,bam,numel(ceil((db-d6)/dz0)+1:ceil((db-d5)/dz0)));
p.B(ceil((db-d5)/dz0)+1:ceil((db-d4)/dz0))=p.A(ceil((db-d5)/dz0)+1:ceil((db-d4)/dz0)).*bam;
...
filename = ['JP_2D_sf_twm',num2str(twm),'ts',num2str(ts),'L',num2str(p.L/1000.),'nx',num2str(p.NX),'W',num2str(p.W/1000.),'nw',num2str(p.NW),'dip',num2str(dip0),'.mat']
p.IC=ceil(p.N/2);
dx=p.L/p.NX;
%Lb_over_dx = Lb/dx
